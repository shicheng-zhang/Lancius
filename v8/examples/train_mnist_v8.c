...
void he_init(double* w, size_t rows, size_t cols) {
    double stddev = sqrt(2.0 / rows);
    for(size_t i=0; i<rows*cols; i++) {
        double u1 = (double)rand() / RAND_MAX; if(u1 < 1e-10) u1 = 1e-10;
        double u2 = (double)rand() / RAND_MAX;
        w[i] = sqrt(-2.0 * log(u1)) * cos(2.0 * M_PI * u2) * stddev;
    }
}
...
int main() {
    srand(42);
    printf("================================================================\n");
    printf("  BASIS V8: MNIST Notepad (Linear Output + Thread Pool)       \n");
    printf("================================================================\n\n");
    ...
    // --- TRAINING GRAPH (Linear Output, No Softmax) ---
    ...
    v8_node* A2 = v8_add(g, Z2, v8_broadcast(g, b2, BATCH_SIZE, 10)); // Raw Logits

    // MSE Loss on Raw Logits (Mathematically stable, avoids Softmax vanishing gradients)
    v8_node* diff = v8_sub(g, A2, Y);
    v8_node* sq = v8_mul(g, diff, diff);
    v8_node* loss = v8_sum(g, sq);
    ...
    v8_pool* pool = v8_pool_create(num_cores);
    ...
    double* w1 = (double*)calloc(784 * 128, sizeof(double)); he_init(w1, 784, 128);
    double* b1_d = (double*)calloc(1 * 128, sizeof(double));
    double* w2 = (double*)calloc(128 * 10, sizeof(double)); he_init(w2, 128, 10);
    ...
    for(uint32_t i=0; i<tg->graph->node_count; i++) {
        ...
        if(n->op == V8_OP_INPUT) {
            if(n->rows == BATCH_SIZE && n->cols == 784) n->runtime_data = x_batch;
            else if(n->rows == BATCH_SIZE && n->cols == 10) n->runtime_data = y_batch;
            else if(n->rows == 784 && n->cols == 128) { n->runtime_data = w1; nW1 = n; }
            else if(n->rows == 1 && n->cols == 128) { n->runtime_data = b1_d; nb1 = n; }
            else if(n->rows == 128 && n->cols == 10) { n->runtime_data = w2; nW2 = n; }
            else if(n->rows == 1 && n->cols == 10) { n->runtime_data = b2_d; nb2 = n; }
        }
    }

    printf("[4/5] Training for %d Epochs (Batch Size: %d) via Thread Pool...\n\n", EPOCHS, BATCH_SIZE);
    ...
    for(int ep=0; ep<EPOCHS; ep++) {
        ...
        for(int i=0; i<=tr_n - BATCH_SIZE; i+=BATCH_SIZE) {
            ...
            for(int b=0; b<BATCH_SIZE; b++) {
                int idx = indices[i+b];
                for(int p=0; p<784; p++) x_batch[b*784 + p] = tr_X[idx*784 + p] / 255.0;
            ...
            v8_schedule_execute(sched, scratch); // Sequential for deterministic training
            step++;

            memcpy(grad_w1, tg->grad_nodes[nW1->id]->runtime_data, 784*128*sizeof(double));
            memcpy(grad_w2, tg->grad_nodes[nW2->id]->runtime_data, 128*10*sizeof(double));
            memcpy(grad_b1, tg->grad_nodes[nb1->id]->runtime_data, 1*128*sizeof(double));
            memcpy(grad_b2, tg->grad_nodes[nb2->id]->runtime_data, 1*10*sizeof(double));

            // Scale gradients by 1/BATCH_SIZE
            for(int k=0; k<784*128; k++) grad_w1[k] /= BATCH_SIZE;
            for(int k=0; k<128*10; k++) grad_w2[k] /= BATCH_SIZE;
            for(int k=0; k<1*128; k++) grad_b1[k] /= BATCH_SIZE;
            for(int k=0; k<1*10; k++) grad_b2[k] /= BATCH_SIZE;
            ...
            epoch_loss += tg->loss_node->runtime_data[0] / (BATCH_SIZE * 10.0); // True MSE
            batches++;
    ...
    v8_node* A2_inf = v8_add(g_inf, Z2_inf, v8_broadcast(g_inf, b2_inf, BATCH_SIZE, 10)); // Raw Logits
    (void)A2_inf;
    ...
    for(int i=0; i<=te_n - BATCH_SIZE; i+=BATCH_SIZE) {
        ...
        for(int b=0; b<BATCH_SIZE; b++) {
            int idx = i+b;
            for(int p=0; p<784; p++) x_batch[b*784 + p] = te_X[idx*784 + p] / 255.0;
        }

        v8_vm_execute(prog, vm_inputs, out_batch, scratch);

        // Argmax over the 10 raw logits
        for(int b=0; b<BATCH_SIZE; b++) {
            int pred = 0; double max_logit = -1e9;
            for(int c=0; c<10; c++) {
                if(out_batch[b*10 + c] > max_logit) {
                    max_logit = out_batch[b*10 + c];
                    pred = c;
                }
            }
    ...
    v8_pool_destroy(pool); // Clean up the Thread Pool
