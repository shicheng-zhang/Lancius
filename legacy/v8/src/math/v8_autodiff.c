...
v8_training_graph* v8_ir_autodiff(v8_graph* fwd_g, v8_node* loss_node) {
    ...
    for(uint32_t i=0; i<fwd_g->node_count; i++) {
        ...
        switch(old->op) {
            ...
            case V8_OP_NOP: n = NULL; break; // V9 Fix: Skip neutralized nodes
            case V8_OP_CONV2D_RELU_FUSED:
                // V12 FIX: Manually allocate to preserve the FUSED opcode!
                n = (v8_node*)v8_arena_alloc(tg->graph->arena, sizeof(v8_node), 8);
                if (n) {
                    memset(n, 0, sizeof(v8_node));
                    n->id = tg->graph->next_id++;
                    n->op = V8_OP_CONV2D_RELU_FUSED; // Crucial: Keep the fused opcode
                    n->ndim = 4;
                    n->input_count = 2;
                    n->inputs = (const v8_node**)v8_arena_alloc(tg->graph->arena, sizeof(v8_node*) * 2, 8);
                    ...
                    if (tg->graph->node_count >= tg->graph->node_cap) {
                        ...
                        tg->graph->nodes = (v8_node**)realloc(tg->graph->nodes, sizeof(v8_node*) * tg->graph->node_cap);
                    }
    ...
    for (int i = fwd_g->node_count - 1; i >= 0; i--) {
        ...
        if (fwd_n->op == V8_OP_NOP) continue; // V9 Fix: Skip neutralized nodes
        ...
        } else if (fwd_n->op == V8_OP_CONV2D_RELU_FUSED) {
            // V9 Fix: Backward pass for fused Conv2D + ReLU
            v8_node* A = fwd_to_full[fwd_n->inputs[0]->id];
            v8_node* W = fwd_to_full[fwd_n->inputs[1]->id];
            v8_node* fused_out = fwd_to_full[fwd_n->id];
            v8_node* masked_grad = v8_relu_bwd(tg->graph, grad_out, fused_out);
            accum_grad(tg->graph, grad_map, fwd_n->inputs[0]->id, v8_conv2d_bwd(tg->graph, masked_grad, A, W, fwd_n->stride, fwd_n->pad), fwd_to_full);
            accum_grad(tg->graph, grad_map, fwd_n->inputs[1]->id, v8_conv2d_bwd_w(tg->graph, masked_grad, A, fwd_n->kernel_h, fwd_n->kernel_w, fwd_n->stride, fwd_n->pad), fwd_to_full);
        }
    }
