...
void v8_execute_vision_op(v8_node* n) {
    ...
    bool is_int8_weight = (n->input_count >= 2 && n->inputs[1] && n->inputs[1]->dtype == V8_DTYPE_INT8);

    if ((n->op == V8_OP_CONV2D || n->op == V8_OP_CONV2D_RELU_FUSED) && is_int8_weight) {
        v8_node* in_node = (v8_node*)n->inputs[0];
        ...
        if (!in && in_node->runtime_data) {
            size_t in_elems = v8_node_elements(in_node);
            double max_val = 0.0;
        ...
        if (n->op == V8_OP_CONV2D_RELU_FUSED) {
