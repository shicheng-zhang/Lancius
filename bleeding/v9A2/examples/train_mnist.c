...
int main() {
    ...
    lancius_node* diff = lancius_sub(g, A2, Y);
    lancius_node* sq = lancius_mul(g, diff, diff);
    lancius_node* sq_sum = lancius_sum(g, sq);
    lancius_node* scale = lancius_const(g, 1.0 / (BATCH_SIZE * 10.0), 1, 1);
    lancius_node* loss = lancius_mul(g, sq_sum, scale);
